...
cleaned_path = "{result_folder}/{clean_params_str}/results/".format(**mconfig)+ "{sample_id}_signal.npy"
summary_path = "{result_folder}/{clean_params_str}/results/".format(**mconfig)+ "{sample_id}_summary.tsv"
figure_path = "{result_folder}/{clean_params_str}/figures/".format(**mconfig)+"{sample_id}"+"_fp={figure_params_str}.png".format(**mconfig)
...
rule view:
  input:
    samples = samples_path,
  params:
    **{**clean_params, "get_features_path": workflow.source_path(
        str(pathlib.Path(workflow.snakefile).parent)+
        "/scripts/common/get_features.py"
      )}
